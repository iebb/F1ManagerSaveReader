...
export default function StaffGeneric({ StaffType = 1 }) {
  ...
  const supportsStatsView = StaffType !== 6;
  ...
  const idColumn = {
    ...
    renderCell: ({ row }) => {
      if (StaffType === 0) {
        return (
          <div className="grid h-9 w-9 place-items-center text-center">
      ...
      return (
        <div className="grid h-9 w-9 place-items-center text-[13px] font-semibold text-white">
          {row.StaffID}
        </div>
      );
    }
    ...
    renderCell: ({ row }) => row.TeamID ? (
      <div className="min-w-0">
        ...
        <div className="mt-0.5 truncate text-[11px] uppercase tracking-[0.08em] text-slate-500">
          {(StaffType === 2 || StaffType === 0) ? (row.TeamFormula <= 3 ? `${StaffType === 2 ? "Car" : "Driver"} #${row.PosInTeam}` : "Reserve") : "Contracted"}
        </div>
      </div>
    ) : row.PreviousTeamID ? (
  ...
  const statsColumns = useMemo(() => {
    const base = [idColumn, natColumn, nameColumn, teamColumn];
    if (StaffType < 5) {
      base.push(ratingColumn);
    }
    if (StaffType === 5) {
      base.push({
        field: 'JobTitle',
        ...
        renderCell: ({ value }) => {
          ...
          return mailSenders[value.replaceAll(/[\[\]]/g, '')] || value;
        }
      });
    }
    base.push(...staffStats.map((x) => ({
    ...
    return base;
  }, [StaffType, staffStats, setEditRow, setSwapRow, version]);

  const contractColumns = useMemo(() => {
    const base = [idColumn, natColumn, nameColumn];
    if (StaffType < 5) {
      base.push(ratingColumn);
    }
    if (StaffType === 5) {
      ...
      base.push(dobColumn, retireColumn, teamColumn, contractColumn);
    }
    base.push(actionsColumn);
    return base;
  }, [StaffType, player.CurrentSeason, version, rows]);
  ...
  return (
    <div className="grid min-w-0 gap-3">
      ...
      <DataGrid
        key={`people-${StaffType}-${supportsStatsView ? view : "contract"}-${updated}`}
        rowHeight={60}
        ...
        columns={supportsStatsView && view === "stats" ? statsColumns : contractColumns}
        initialState={{
          pagination: {
